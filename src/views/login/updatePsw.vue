<template>
    <cus-nav :title="$t('修改登录密码')"></cus-nav>
    <div class="pl30 pr30 pt40">

        <div class="size28 mb30">{{ $t('旧登录密码') }}</div>
        
        <CusInput :tips="$t('请输入旧的登录密码')" :psw="true" v-model="oldPassword"></CusInput>

        <div class="mt30 size28 mb30">{{ $t('新密码') }}</div>

        <CusInput :tips="$t('请设置新的登录密码')" :psw="true" v-model="newPassword"></CusInput>
...
const submit = () => {
    if(!oldPassword.value)return showToast(t('请输入旧的登录密码'))
    if(!newPassword.value)return showToast(t('请设置新的登录密码'))
    ...
    apiPost('/api/users/my/change_password',{
        old_password:oldPassword.value,
        new_password:newPassword.value
    }).then(()=>{
        showSuccessToast(t('修改成功，请重新登录'))
        setTimeout(() => {
            routerReplace('/login')
        }, 1200);
    })
